...
<?php if (DISPLAY_BANNER): ?>
    <div style="background: #2e89fc;<?php if (is_rtl()): ?>margin-right<?php else: ?>margin-left<?php endif; ?>: -20px;padding: 10px 21px;">
        <img src="<?php echo plugin_dir_url(__FILE__); ?>../assets/images/protected-page-logo-white.png"
    ...
    <div class="wrap">
        ...
        <h2 class="nav-tab-wrapper">
            <?php foreach ($supported_post_types as $post_type => $post_label): ?>
                <a href="<?php echo admin_url(); ?>admin.php?page=protected-page&post_type=<?php echo esc_html($post_type) ?>"
                   class="nav-tab <?php if ($post_type === $selected_post_type): ?>nav-tab-active<?php endif; ?>"><?php echo esc_html($post_label) ?></a>
            <?php endforeach; ?>
                    ...
                    <tr class="<?php if ($page->is_page_protected): ?>is-protected<?php endif; ?>">
                        ...
                        <td>
                            <section class="protected-page-switcher is-page-protected">
                                <input type="checkbox" name="protected" id="switcher-<?php echo $key; ?>"
                                       data-page_id="<?php echo esc_attr($page->ID); ?>"
                                       data-page_title="<?php echo esc_attr($page->post_title); ?>"
                                       <?php if ($page->is_page_protected): ?>checked<?php endif; ?>/>
                        ...
                        <td>
                            <a href="<?php echo admin_url(); ?>admin.php?page=<?php echo $this->parent_menu_slug ?>&page_id=<?php echo esc_html($page->ID); ?>"
                               class="button button-primary generate-password-link"><?php echo __('Generate password', 'protected-page'); ?></a>
                        </td>
                    </tr>
