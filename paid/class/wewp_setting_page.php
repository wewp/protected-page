...
if(!class_exists('Wewp_Setting_Page')){
	class Wewp_Setting_Page
	{
		...
		public function render_support_post_type()
		{
            ...
            <div>
                <input id="post_type_<?php esc_attr($post_name);?>"
                       type="checkbox"
                       name="wewp_protected_page_cpt_support[<?php echo esc_attr($post_name);?>]"
                       value="<?php echo esc_html($post_type_options->label);?>"
				       <?php if (isset($post_types_supported[$post_name])): ?>checked<?php endif;?>
                />
		...
		public function render_top_bar_button()
		{
            ...
            <div>
                <input type="checkbox"
                       name="wewp_protected_page_top_bar_button"
                       id="wewp_protected_page_top_bar_button"
                       value="1"
					<?php checked(1, $display_admin_top_bar_button,true); ?>
                />
            </div>
			<?php
		}
		...
		<?php endif;?>
            <div class="wrap">
                <h1 class="wp-heading-inline"><?php echo __('Settings', 'protected-page'); ?></h1>
                <hr>
